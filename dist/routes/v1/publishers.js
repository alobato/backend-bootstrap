...
import { CreatePublisherSchema, GetPublisherSchema, SearchPublishersSchema, UpdatePublisherSchema, } from "../../services/schemas.js";
export function getPublishers(_req, res) {
    PublisherService.getAllPublishers()
        .then((publishers) => res.json(publishers))
        .catch((err) => res.status(500).json({ error: err.message }));
}
export function getPublisherById(req, res) {
    ...
    PublisherService.getPublisher(parsed.data)
        .then((publisher) => publisher ? res.json(publisher) : res.status(404).json({ error: "Editora não encontrada" }))
        .catch((err) => res.status(500).json({ error: err.message }));
}
export function searchPublishers(req, res) {
