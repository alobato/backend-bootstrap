...
import type {
  AddAuthorsToBookSchema,
  AddCategoriesToBookSchema,
  CreateBookSchema,
  GetBookSchema,
  RemoveAuthorsFromBookSchema,
  RemoveCategoriesFromBookSchema,
  SearchBooksSchema,
  UpdateBookSchema,
} from "./schemas.js";
...
export const BookService = {
  ...
  async getBook(input: z.infer<typeof GetBookSchema>) {
    const { bookId } = input;
    return this.getBookById(bookId);
  },

  async searchBooks(input: z.infer<typeof SearchBooksSchema>) {
    const { query } = input;
  ...
  async createBook(input: z.infer<typeof CreateBookSchema>) {
  ...
  async updateBook(id: number, input: z.infer<typeof UpdateBookSchema>) {
    const [book] = await db.select().from(books).where(eq(books.id, id)).limit(1);
  ...
  async addAuthorsToBook(input: z.infer<typeof AddAuthorsToBookSchema>) {
  ...
  async removeAuthorsFromBook(input: z.infer<typeof RemoveAuthorsFromBookSchema>) {
    const { bookId, authorIds } = input;
  ...
  async addCategoriesToBook(input: z.infer<typeof AddCategoriesToBookSchema>) {
  ...
  async removeCategoriesFromBook(input: z.infer<typeof RemoveCategoriesFromBookSchema>) {
    const { bookId, categoryIds } = input;
  ...
  async getBookCategories(bookId: number) {
