...
export const UserService = {
  ...
  async updateUser(id: number, input: z.infer<typeof UpdateUserSchema>) {
    const [user] = await db.select().from(users).where(eq(users.id, id)).limit(1);
    ...
    const now = new Date().toISOString();
    const updateData: Partial<typeof users.$inferInsert> = { updatedAt: now };

    if (input.name !== undefined) updateData.name = input.name;
    if (input.email !== undefined) updateData.email = input.email.trim().toLowerCase();
    if (input.role !== undefined) updateData.role = input.role;
    if (input.password !== undefined) updateData.password = await bcrypt.hash(input.password, 10);
